...
export function baseHeaderEq(
    a: { block: CalloutBlock; collapsed: boolean },
    b: { block: CalloutBlock; collapsed: boolean }
): boolean {
    return (
        a.block.type === b.block.type &&
        a.block.title === b.block.title &&
        a.collapsed === b.collapsed &&
        a.block.headerFrom === b.block.headerFrom &&
        a.block.hasContent === b.block.hasContent
    );
}
...
export function buildCollapseToggle(
    ...
    const doToggle = (e: Event) => {
        ...
        setCollapseState(collapseKey, newCollapsed);
        view.dispatch({
...
export function buildIconSpan(typeId: string): HTMLSpanElement {
    const iconSpan = document.createElement("span");
    iconSpan.className = "cm-callout-icon";
    const svgIcon = createIconSVG(typeId);
    if (svgIcon) {
